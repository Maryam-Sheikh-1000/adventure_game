...
async function welcome(){
    let rainbowTitle= chalkAnimation.neon("Welcome to Adventure Game");
    await sleep();
    rainbowTitle.stop();
...
async function AdventureGame() {
    ...
    const WarQues:QuestionInterface={name:'WarInput',type:'list',choices:['Attack','Run']};
                ...
                do{
                    battleResponse=String((await inquirer.prompt(Questions[2])).battleInput);
                        ...
                        if(enemyHealth<=0 && battleResponse !='Run'){
                            console.log(`\t --> You defeated ${currentEnemy}, congratulation! your score :${scorePoints+=10} <--\n`);
